...
public class JaxbMovieReader implements MovieReader{
	...
	public List<Movie> loadMovies() {
		...
		try {
			final JAXBContext jaxb = JAXBContext.newInstance(MovieMetadata.class); // 작스비 Context 객체
	...
	public static class MovieMetadata {
		...
		public List<Movie> toMovies() {
			return movies.stream().map(MovieData::toMovie).collect(Collectors.toList()); // 메타데이터 -> 무비 객체
		}
	}
